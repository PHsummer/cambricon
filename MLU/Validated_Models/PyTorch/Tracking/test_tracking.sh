...
function usage
{
    ...
    echo "|  required param1: 0)GOTURN,         1)MDNet,        2)Siamese"
    echo "|  required param2: precision: fp32, O0, O1, O2, O3, amp "
    echo "|                   device: mlu, gpu            "
    echo "|                   option1: ddp                "
    echo "|  required param3: device nums                 "
    echo "|  eg. ./test_tracking.sh 0 fp32-mlu"
    echo "|      which means running GOTURN on single MLU card with fp32 precision."
    echo "|  eg. export MLU_VISIBLE_DEVICES=0,1,2,3 && ./test_tracking.sh 1 fp32-mlu-ddp"
    echo "|      which means running MDNet on 4 MLU cards with fp32 precision."
    echo "-------------------------------------------------------------"
}
