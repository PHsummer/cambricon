...
function usage
{
    ...
    echo "|  required param1: 0)Bert,   1)MacBert,   2)RoBERTa,    3)KG_Bert"
    echo "|  required param2: precision: fp32, O0, O1, O2, O3, amp "
    echo "|                   device: mlu, gpu            "
    echo "|                   option1: ddp                "
    echo "|  required param3: device nums                 "
    echo "|  eg. ./test_languagemodeling.sh 0 fp32-mlu"
    echo "|      which means running Bert on single MLU card with fp32 precision."
    echo "|  eg. export MLU_VISIBLE_DEVICES=0,1,2,3 && ./test_languagemodeling.sh 1 fp32-mlu-ddp"
    echo "|      which means running Bert on 4 MLU cards with fp32 precision."
    echo "-------------------------------------------------------------"
}
...
# get location of net
net_list=( Bert MacBert RoBERTa KG_Bert )
