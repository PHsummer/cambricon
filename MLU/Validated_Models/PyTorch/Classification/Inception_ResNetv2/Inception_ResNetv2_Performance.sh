...
then
  if [[ ${dev_type} == "gpu" ]]; then
    docker exec -it "${CONT_NAME}" bash -c "python train.py /data/imagenet_training --sched cosine --epochs 1 --batch-size ${batch_size} --iters 300 --model inception_resnet_v2 --epochs 1 --cooldown-epochs 0" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
  elif [[ ${dev_type} == "mlu"  ]]; then
    docker exec -it "${CONT_NAME}" bash -c "source /torch/venv3/pytorch/bin/activate && python train.py /data/imagenet_training --sched cosine --epochs 1 --batch-size ${batch_size} --iters 300 --model inception_resnet_v2 --epochs 1 --cooldown-epochs 0" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
  fi
else
  if [[ ${dev_type} == "gpu" ]]; then
    docker exec -it "${CONT_NAME}" bash -c "python -m torch.distributed.launch --nproc_per_node=${device_count} train.py /data/imagenet_training --sched cosine --epochs 1 --cooldown-epochs 0 --batch-size ${batch_size} --iters 300 --model inception_resnet_v2 --epochs 1" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log    
  elif [[ ${dev_type} == "mlu"  ]]; then
    docker exec -it "${CONT_NAME}" bash -c "source /torch/venv3/pytorch/bin/activate && python -m torch.distributed.launch --nproc_per_node=${device_count} train.py /data/imagenet_training --sched cosine --epochs 1 --cooldown-epochs 0 --batch-size ${batch_size} --iters 300 --model inception_resnet_v2 --epochs 1" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log    
  fi
fi
