...
function usage
{
    ...
    echo "|  required param1: 0)centernet,     1)FaceBox,  2) mobilenetv2_ssdlite,  3) pvanet"
    echo "|  required param2: precision: fp32, O0, O1, O2, O3, amp "
    echo "|                   device: mlu, gpu            "
    echo "|                   option1: ddp                "
    echo "|  required param3: device nums                 "
    echo "|  eg. ./test_detection.sh 1 fp32-mlu"
    echo "|      which means running ecanet on single MLU card with fp32 precision."
    echo "|  eg. export MLU_VISIBLE_DEVICES=0,1,2,3 && ./test_detection.sh 1 fp32-mlu-ddp"
...
# get location of net
net_list=(  CenterNet       FaceBox    MobileNetV2_SSDlite    PVANet
            )
