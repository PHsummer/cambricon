...
def prepare_tokenizer(args):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        if USE_MLU:
            token_counts = torch.LongTensor([after, eod_token]).to("mlu")
        else:
            token_counts = torch.cuda.LongTensor([after, eod_token])
    else:
        if USE_MLU:
            token_counts = torch.LongTensor([0, 0]).to("mlu")
        else:
    ...
    num_tokens = token_counts[0].cpu().item()
    eod_token = token_counts[1].cpu().item()
