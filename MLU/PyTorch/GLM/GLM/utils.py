...
def get_spare_port(args):
    if torch.distributed.get_rank() == 0:
        ...
        if port == args.master_port:
            ...
            port = int(port.strip())
        if USE_MLU:
            port = torch.LongTensor([port])
        else:
            port = torch.cuda.LongTensor([port])
    else:
        if USE_MLU:
            port = torch.LongTensor([0])
        else:
...
def report_memory(name):
    ...
    if USE_MLU:
        string += ' | allocated: {}'.format(
            ct.memory_allocated() / mega_bytes)
        string += ' | max allocated: {}'.format(
            ct.max_memory_allocated() / mega_bytes)
        string += ' | cached: {}'.format(ct.memory_cached() / mega_bytes)
    else:
        string += ' | allocated: {}'.format(
    ...
    if args.deepspeed and not no_deepspeed:
        ...
        if mpu.get_data_parallel_rank() == 0:
            ...
            if not args.no_save_rng:
                ...
                if USE_MLU:
                    # print("mlu does not support get_cuda_rng_state")
                    pass
                else:
