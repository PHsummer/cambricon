...
try:
    if getattr(torch.mlu.amp, 'autocast') is not None:
        has_native_amp = True
...
def mlu_timestamp(sync=False, device=None):
    if sync:
        torch.mlu.synchronize(device=device)
    return time.perf_counter()
...
class BenchmarkRunner:
    def __init__(
            self, model_name, detail=False, device='mlu', torchscript=False, precision='float32',
        ...
        self.amp_autocast = torch.mlu.amp.autocast if self.use_amp else suppress
        ...
        if 'mlu' in self.device:
            self.time_fn = partial(mlu_timestamp, device=self.device)
        else:
...
class InferenceBenchmarkRunner(BenchmarkRunner):
    ...
    def __init__(self, model_name, device='mlu', torchscript=False, **kwargs):
...
class ProfileRunner(BenchmarkRunner):
    ...
    def __init__(self, model_name, device='mlu', profiler='', **kwargs):
