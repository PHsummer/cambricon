    ...
    def __call__(self, state, x):
        ...
        if self.typ == "lstm":
            ...
            if state["h"][0] is None:
                xp = self.xp
                with chainer.backends.mlu.get_device_from_id(self._device_id):
...
class BPTTUpdater(training.updaters.StandardUpdater):
    ...
    def update_core(self):
        ...
        for _ in range(self.accum_grad):
            ...
            xp = chainer.backends.mlu.get_array_module(x)
            loss = 0
            state = None
...
def train(args):
    ...
    # check mlu and cudnn availability
    if not chainer.mlu.available:
        logging.warning("mlu is not available")
    if not chainer.mlu.cudnn_enabled:
    ...
    if args.ngpu > 0:
        ...
        gpu_id = 0
        chainer.mlu.get_device_from_id(gpu_id).use()
        model.to_gpu()
    else:
        gpu_id = -1
