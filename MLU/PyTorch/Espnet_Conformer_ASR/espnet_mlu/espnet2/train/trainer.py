...
class Trainer:
    ...
    def resume(
        ...
        states = torch.load(
            checkpoint,
            map_location=f"mlu:{torch.mlu.current_device()}" if ngpu > 0 else "cpu",
        )
        '''
    ...
    def run(
        ...
        skip_valid = trainer_options.max_epoch - 100
        ...
        for iepoch in range(start_epoch, trainer_options.max_epoch + 1):
            ...
            if (iepoch == skip_valid):
                reporter = Reporter(iepoch)
            else:
            ...
            if not distributed_option.distributed or distributed_option.dist_rank == 0:
                ...
                #torch.save(model, output_dir / f"{iepoch}epoch_full.pth")
                ...
                for e in range(1, iepoch):
                    ...
                    q = output_dir / f"{e}epoch_full.pth"
                    if q.exists() and e not in nbests:
                        q.unlink()
    ...
    def train_one_epoch(
        ...
        iterator_stop = torch.tensor(0).to("mlu" if ngpu > 0 else "cpu")

        #with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU,torch.profiler.ProfilerActivity.MLU,],
        #                            schedule=torch.profiler.schedule(wait=5,warmup=4,active=1),
        #                            on_trace_ready=torch.profiler.tensorboard_trace_handler("./tensorboard/"), with_stack=True) as prof:                        
        start_time = time.perf_counter()
        for iiter, (utt_id, batch) in enumerate(
            ...
            batch = to_device(batch, "mlu" if ngpu > 0 else "cpu")
            if no_forward_run:
