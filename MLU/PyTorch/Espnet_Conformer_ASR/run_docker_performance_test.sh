...
docker build -t conformer_pytorch_1.9.0_docker:final -f Dockerfile ./espnet_mlu
...
docker exec -ti --use-nas-user ${CONT_NAME} /bin/bash -c \
        ...
        ./run.sh --ngpu 16 --skip_data_prep false --skip_eval true --lm_args '--max_epoch 1 --num_iters_per_epoch 200' --asr_args '--max_epoch 1 --num_iters_per_epoch 200'"
docker container rm -f ${CONT_NAME} || true

echo "LM training performance"
lm_log_path=$OUTPUT_DIR/lm_train_lm_transformer_zh_char_max_epoch1_num_iters_per_epoch200/train.log
batch_bins=$(grep "batch_bins" $lm_log_path | head -n 1 | grep -o "batch_bins=[.0-9]*" | grep -o "[.0-9]*")
train_time=$(grep "1epoch results" $lm_log_path | grep -o "train_time=[.0-9]*" | grep -o "[.0-9]*")
...
asr_log_path=$OUTPUT_DIR/asr_train_asr_conformer_mlu_raw_zh_char_max_epoch1_num_iters_per_epoch200_sp/train.log
batch_bins=$(grep "batch_bins" $asr_log_path | head -n 1 | grep -o "batch_bins=[.0-9]*" | grep -o "[.0-9]*")
train_time=$(grep "1epoch results" $asr_log_path | grep -o "train_time=[.0-9]*" | grep -o "[.0-9]*")
