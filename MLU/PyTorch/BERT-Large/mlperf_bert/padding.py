...
def generate_mask(attention_mask, heads, pad=False, fuse_mask=True, unpad_fmha=False):
    ...
    if pad==False and fuse_mask == True:
        ...
        mask = torch.zeros([ntokens], device="mlu", dtype=torch.float16)
        ...
        mask = torch.zeros([len(indices_mask)], device="mlu", dtype=torch.float16)
...
class PadInput(torch.autograd.Function):
    ...
    def forward(ctx, input, indices, batch, maxseqlen, hidden, ntokens):
        ...
        output = torch.zeros([ntokens,hidden], device="mlu", dtype=torch.float16)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros([ctx.ntokens,ctx.hidden], device="mlu", dtype=torch.float16)
