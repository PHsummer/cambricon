...
class BertPreTrainedModel(nn.Module):
    ...
    def init_bert_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            # module.weight.data.copy_(module.weight.data.cpu().normal_(mean=0.0, std=self.config.initializer_range))
...
#                mlm_predictions = prediction_scores.argmax(dim=-1).view(-1)
                ...
                mlm_acc = torch.true_divide((mlm_predictions == mlm_labels).sum(dtype=torch.float), num_valid)
