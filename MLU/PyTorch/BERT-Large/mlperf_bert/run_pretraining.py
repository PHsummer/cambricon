...
class pretraining_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # pytorch1.6 dosen`t have "torch.count_nonzero" function, use "torch.nonzero" instead.
        #masked_token_count = torch.count_nonzero(masked_lm_positions)
        masked_token_count = torch.nonzero(masked_lm_positions, as_tuple=False).size()[0]
...
def prepare_model_and_optimizer(args, device):
    ...
    model.to(device)
    model.heads_only_segment.cls.predictions.decoder.weight = model.bert_model_segment.bert.embeddings.word_embeddings.weight
  ...
  #                          value=args.learning_rate, sync=False)
    if args.distributed_lamb:
        ...
        if args.use_mlu:
            from cnmix.optimizers import FusedLAMB
        else:
...
#            optimizer._init_everything()
            ...
            if args.use_mlu:
                amp.core.handle._handle.loss_scalers[0]._loss_scale = float(os.getenv("INIT_LOSS_SCALE", 2**20))
            else:
    ...
    if args.distributed_lamb:
        ...
        if args.bypass_amp:
            ...
            if args.use_mlu:
                import cnmix as amp
                #global_step += 1
                global_step += 0 if  amp.core.handle._handle.loss_scalers[0]._has_overflow else 1
                if amp.core.handle._handle.loss_scalers[0]._has_overflow:
                    print("Gradient overflow.  Skipping step, loss scaler 0 reducing loss scale to : %f" % amp.core.handle._handle.loss_scalers[0]._loss_scale)
            else:
                from apex import amp
...
def exchange_padding_fast(device, input_ids, segment_ids, input_mask, masked_lm_labels, next_sentence_labels, max_batch_size):
    ...
    torch.distributed.all_gather(tensors_, torch.ops.torch_mlu.view(tensors, torch.float16))

    #tensors_ = torch.stack(tensors_).view(torch.int16).long()
    tensors_ = torch.ops.torch_mlu.view(torch.stack(tensors_), torch.int16).long()
...
#            assert use_DDP, "Running multi-GPU without --use_ddp"
        ...
        if use_cuda_graph and args.cuda_graph_mode=='full_iteration':
            ...
            if args.use_mlu:
                import torch_mlu.core.mlu_model as ct
                current_device = torch.device('mlu', ct.current_device())
            else:
...
#                skipped_steps = torch.zeros((1,), dtype=torch.int, device=current_device)
        ...
        while global_step < args.max_steps and not end_training:
            ...
            train_sampler = RandomSampler(train_data)
            train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                          batch_size=args.train_batch_size, num_workers=4 if args.train_batch_size<=8 else 4, worker_init_fn=worker_init, pin_memory=True)
...
#                optimizer._init_everything()
            ...
            for f_id in range(f_start_id + 1, len(files)):
                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    #if step == 50:
                    #    dump_cnnl_gencase(enable=True, level='L1')
                    ...
                    if args.log_freq > 0 and training_steps % (args.log_freq * args.gradient_accumulation_steps) == 0:
                        ...
                        if utils.is_main_process():
                            ...
                            # print("hardware time: %s" % hardware_time_ms)
                            time_interval = time.time() - now_time
                            #print("e2e: ", time_interval)
                            # start.place()
                            ...
                            if args.use_mlu:
                                import torch_mlu.core.mlu_model as ct
                                max_allocated=ct.max_memory_allocated()
                                max_cached=ct.max_memory_cached()
                            else:
                                max_allocated=torch.cuda.max_memory_allocated()
                                max_cached = torch.cuda.max_memory_reserved()
                            max_allocated /= (1024 * 1024)
                            max_cached /= (1024 * 1024)
                           ...
                           #           "timestamp": now_time})
                            ...
                            print("[MLU] global_step: %d, step_loss: %.4f, avg_loss: %.2f, lr: %s, throughput: %.2f seq/s, time_interval: %.2f s, max_memory_allocated: %.2f M, max_memory_cached: %.2f M" %
                                  (global_step,
                                   loss.item() * args.gradient_accumulation_steps / divisor,
                                   average_loss / (args.log_freq * divisor), now_lr,
                                   training_perf,
                                   time_interval,
                                   max_allocated,
                                   max_cached
                    ...
                    if global_step >= args.max_steps or end_training:
                        ...
                        if args.use_mlu:
                            average_loss = torch.tensor(average_loss, dtype=torch.float32).to('mlu')
                        else:
