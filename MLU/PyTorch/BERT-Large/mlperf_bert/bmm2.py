...
class Bmm2Function(torch.autograd.Function):
    ...
    def forward(ctx, batch1, batch2, seqlen, batch, maxseqlen, heads, embed, sync, stream):
        ...
        output = torch.empty([ntokens,heads,embed], device="mlu", dtype=torch.float16)
    ...
    def backward(ctx, grad_output):
        ...
        grad_batch1 = torch.empty([ntokens2*heads], device="mlu", dtype=torch.float16)
        grad_batch2 = torch.empty([ntokens,heads*embed], device="mlu", dtype=torch.float16)
...
class Bmm2StridedFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_batch1 = torch.empty(ntokens2*heads, device="mlu", dtype=torch.float16)
        grad_mixed = torch.empty([ntokens,heads*3*embed], device="mlu", dtype=torch.float16)
