...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if is_device_available['mlu']:
            LOGGER.info(('\n' + '%10s' * 6) %
                        ('Epoch', 'box', 'obj', 'cls', 'labels', 'img_size'))
        else:
            LOGGER.info(('\n' + '%10s' * 7) % ('Epoch', 'gpu_mem',
        ...
        if RANK in [-1, 0] and (epoch >= (epochs-50)):
            # mAP
            ...
            if f.exists():
                ...
                if f is best and device.type != 'mlu':
                    ...
                    results, _, _ = val.run(data_dict,
                                            ...
                                            model=attempt_load(
                                                f, device).half(),
                                            iou_thres=0.50 if is_coco else 0.60,  # best pycocotools results at 0.5
                                            single_cls=single_cls,
                                            dataloader=val_loader,
                                            save_dir=save_dir,
                                            save_json=is_coco,
...
def parse_opt(known=False):
    ...
    parser.add_argument('--pyamp', action='store_true',
                        help='using amp for mixed precision training')
    parser.add_argument('--iters', type=int, default=-1,
                        help="Total iters for benchmark.")
    parser.add_argument('--skip', action='store_true',
                        help='skip val or save pt.')
