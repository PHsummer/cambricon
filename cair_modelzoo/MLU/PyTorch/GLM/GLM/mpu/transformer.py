...
class GPT2ParallelTransformer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if deepspeed.checkpointing.is_configured():
            ...
            get_cuda_rng_tracker = deepspeed.checkpointing.get_cuda_rng_tracker
            if USE_MLU:
                checkpoint = deepspeed.cndsp.runtime.activation_checkpointing.checkpointing.checkpoint
            else:
                checkpoint = deepspeed.checkpointing.checkpoint

    def forward(self, hidden_states, position_ids, attention_mask, memory_states=None,
                encoder_states=None, return_memory=False, detach_memory=True):
        ...
        if self.checkpoint_activations:
            ...
            while l < num_layers:
                args = [hidden_states, attention_mask] \
                       if not self.use_decoder_layer else [hidden_states,
                       encoder_states, attention_mask]
                if self.relative_encoding:
                ...
                # hidden_states = self.layers[0](*args, mem=mem_i)
                l += chunk_length
        else:
