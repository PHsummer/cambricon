...
try:
    if torch.is_mlu_available():
        ...
        print("[ERROR]: import torch_mlu failed")
    ...
    while args.iteration < args.train_iters:
        ...
        if args.eval_interval and args.iteration % args.eval_interval == 0 and args.do_valid:
            ...
            evaluate_and_print_results(
                ...
                verbose=False, step=args.iteration, summary_writer=summary_writer,
                forward_step_func=forward_step)
    ...
    if USE_MLU:
        loss_data = torch.FloatTensor(
             [total_lm_loss, total_gpt_loss, total_bert_loss, total_sent_loss,
              total_multi_loss, gpt_iters, bert_iters, sent_iters, multi_iters]).to("mlu")
    else:
...
def initialize_distributed(args):
    ...
    if USE_MLU:
        device = args.rank % ct.device_count()
    else:
...
def set_random_seed(seed):
    ...
    if seed is not None and seed > 0:
        ...
        if USE_MLU:
            print("WARNING: MLU does not set parallel random seed")
        else:
...
def get_train_val_test_data(args, tokenizer):
    ...
    if mpu.get_model_parallel_rank() == 0:
        ...
        if USE_MLU:
            data_counts = torch.LongTensor([int(args.do_train), int(args.do_valid),
                                            int(args.do_test)]).to("mlu")
        else:
            data_counts = torch.cuda.LongTensor([int(args.do_train), int(args.do_valid),
                                                 int(args.do_test)])
    else:
        if USE_MLU:
            data_counts = torch.LongTensor([0, 0, 0]).to("mlu")
        else:
