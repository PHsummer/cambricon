...
def get_dataset(name, tokenizer, pre_tokenize, data_parallel_rank, loader_scatter=None,
                no_lazy_loader=False, half_lazy_loader=False):
    ...
    if issubclass(dataset, corpora.PromptReader):
        if not (exists_lazy(path, data_type='prompt')    \
           ...
           and not (loader_scatter is not None
        ...
        if loader_scatter is not None:
            if not (exists_scatter(path, data_type='prompt', scatter_num=loader_scatter) \
               and exists_scatter(path, data_type='text', scatter_num=loader_scatter)):
                if global_rank == 0:
                    ...
                    for i in range(loader_scatter):
                        ...
                        prompt_writer = LazyWriter(scatter_path, data_type='prompt',
                                                   is_array=pre_tokenize)
                        text_writer = LazyWriter(scatter_path, data_type='text',
                                                 is_array=pre_tokenize)
                    ...
                    while not (exists_scatter(path, data_type='prompt',
                               scatter_num=loader_scatter) \
                          and exists_scatter( path, data_type='text',
                              scatter_num=loader_scatter)):
                        time.sleep(1)
            scatter_path = get_scatter_path(path,
            ...
            texts = LazyLoader(path, data_type='text', map_fn=map_fn, mem_map=True,
                               is_array=pre_tokenize, load_memory=no_lazy_loader,
                               half_load=half_lazy_loader)
        text = corpora.PromptDataset(prompt_loader=prompts, text_loader=texts,
                                     tokenizer=tokenizer, to_tokenize=not pre_tokenize)
        if loader_scatter is None:
            if global_rank == 0:
                print(f"=====Create dataset {name} with {len(text)} documents")
                sample_tokens = text[0]['tokens'][:128]
                #for i in range(10):
            ...
            for scatter_id in range(loader_scatter):
                if data_parallel_rank % loader_scatter == scatter_id \
                   and data_parallel_rank // loader_scatter == 0:
                    ...
                    for i in range(10):
                        ...
                        print(sample_tokens)
                        print(tokenizer.DecodeIds(sample_tokens).encode('utf-8'))
                # the barrier blocks printing message, therefore commented
...
def make_dataset(path, seq_length, mem_length, shuffle=True, split=None, tokenizer=None,
                 sample_one_document=False, pre_tokenize=False, ds_type='', save_splits=None,
                 load_splits=None, save_test_data=None, no_lazy_loader=False,
                 loader_scatter=None, data_parallel_rank=None, filter_english=False,
                 non_sentence_start=0.0, half_lazy_loader=False, **kwargs):
    ...
    if isinstance(path, str):
        ds = get_dataset(path, tokenizer=tokenizer, pre_tokenize=pre_tokenize,
                         no_lazy_loader=no_lazy_loader, loader_scatter=loader_scatter,
                         data_parallel_rank=data_parallel_rank,
                         half_lazy_loader=half_lazy_loader)
    else:
        ds = [get_dataset(p, tokenizer=tokenizer, pre_tokenize=pre_tokenize,
                          no_lazy_loader=no_lazy_loader, loader_scatter=loader_scatter,
                          data_parallel_rank=data_parallel_rank,
    ...
    def wrap_dataset(dataset):
        if ds_type.lower() == 'bert':
            ...
            dataset = BertSentencepairDataset(dataset, max_seq_len=seq_length,
    ...
    if should_split(split):
        ds = split_ds(ds, split, shuffle=shuffle, save_splits=save_splits,
