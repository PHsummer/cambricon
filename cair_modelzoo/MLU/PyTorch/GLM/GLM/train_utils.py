...
try:
    if torch.is_mlu_available():
        ...
        USE_MLU = True
        from cnmix.optimizers import FusedAdam as Adam
except :
...
import mpu
from mpu.random import see_memory_usage
...
def setup_model_and_optimizer(args, model_type=None, multi_token=True, num_labels=None, spell_length=None):
    ...
    model = get_model(args, model_type=model_type, multi_token=multi_token,
                      num_labels=num_labels, spell_length=spell_length)
...
def train_step(data_iterator, model, optimizer, lr_scheduler, args, timers,
               forward_step_func, mems=None, single_step=False):
    ...
    while True:
        ...
        if USE_MLU:
            torch.distributed.all_reduce(reduced_loss, group=mpu.get_data_parallel_group())
            reduced_loss.data = reduced_loss / (args.world_size / args.model_parallel_size)
        else:
