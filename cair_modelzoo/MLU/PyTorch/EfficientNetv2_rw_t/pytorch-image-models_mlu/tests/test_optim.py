...
def _test_state_dict(weight, bias, input, constructor):
    ...
    def fn_base(optimizer, weight, bias):
        optimizer.zero_grad()
        i = input_mlu if weight.is_mlu else input
    ...
    input_mlu = Variable(input.data.float().mlu())
    weight_mlu = Variable(weight.data.float().mlu(), requires_grad=True)
    bias_mlu = Variable(bias.data.float().mlu(), requires_grad=True)
    optimizer_mlu = constructor(weight_mlu, bias_mlu)
    fn_mlu = functools.partial(fn_base, optimizer_mlu, weight_mlu, bias_mlu)

    state_dict = deepcopy(optimizer.state_dict())
    state_dict_c = deepcopy(optimizer.state_dict())
    optimizer_mlu.load_state_dict(state_dict_c)
    ...
    for _i in range(20):
        optimizer.step(fn)
        optimizer_mlu.step(fn_mlu)
        torch_tc.assertEqual(weight, weight_mlu)
        torch_tc.assertEqual(bias, bias_mlu)
