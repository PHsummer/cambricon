...
if [[ ${dev_type} == "gpu" ]]; then
  docker exec -it "${CONT_NAME}" bash -c "python tools/model_converters/beit2mmseg.py https://dl.fbaipublicfiles.com/mae/pretrain/mae_pretrain_vit_base.pth pretrain/mae_pretrain_vit_base_mmcls.pth && ./tools/dist_train.sh ./configs/mae/upernet_mae-base_fp16_8x2_512x512_160k_ade20k.py ${device_count} /data/ADE20K/ ${batch_size} 0 10 1" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
elif [[ ${dev_type} == "mlu"  ]]; then
  docker exec -it "${CONT_NAME}" bash -c "source /torch/venv3/pytorch/bin/activate \
  && ./tools/dist_train.sh ./configs/mae/upernet_mae-base_fp16_8x2_512x512_160k_ade20k.py  ${device_count} /data/CityScapes/ ${batch_size} 0 10 1" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
