...
if [[ ${dev_type} == "gpu" ]]; then
  docker exec -it "${CONT_NAME}" bash -c "./tools/dist_train.sh ./configs/vit/upernet_deit-b16_512x512_80k_ade20k.py ${device_count} /data/ADE20K/ ${batch_size} 0 10 1" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
elif [[ ${dev_type} == "mlu"  ]]; then
  docker exec -it "${CONT_NAME}" bash -c "source /torch/venv3/pytorch/bin/activate && ./tools/dist_train.sh ./configs/vit/upernet_deit-b16_512x512_80k_ade20k.py ${device_count} /data/ADE20K/ ${batch_size} 0 10 1" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
fi
