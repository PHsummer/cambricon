...
if [[ ${dev_type} == "gpu" ]]; then
  docker exec -it "${CONT_NAME}" bash -c "./tools/dist_train.sh ./configs/ssd/ssdlite_mobilenetv2_scratch_600e_coco.py ${device_count} /data/COCO2017/ ${batch_size} 1 0 50" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
elif [[ ${dev_type} == "mlu" ]]; then
  docker exec -it "${CONT_NAME}" bash -c "source /torch/venv3/pytorch/bin/activate && ./tools/dist_train.sh ./configs/ssd/ssdlite_mobilenetv2_scratch_600e_coco.py ${device_count} /data/COCO2017/ ${batch_size} 1 0 50" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
fi
