...
if [[ ${dev_type} == "gpu" ]]; then
  docker exec -it "${CONT_NAME}" bash -c "mkdir models && python train.py --train_path ./omniglot/python/images_background --test_path ./omniglot/python/images_evaluation --gpu_nums ${device_count} --model_path models --batch_size ${batch_size} --max_iter 100" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
elif [[ ${dev_type} == "mlu"  ]]; then
  docker exec -it "${CONT_NAME}" bash -c "source /torch/venv3/pytorch/bin/activate && mkdir models && python train.py --train_path ./omniglot/python/images_background --test_path ./omniglot/python/images_evaluation --gpu_nums ${device_count} --model_path models --batch_size ${batch_size} --max_iter 100" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
fi
