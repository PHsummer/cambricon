...
then
  if [[ ${dev_type} == "gpu" ]]; then
    docker exec -it "${CONT_NAME}" bash -c "export CUDA_VISIBLE_DEVICES=0 && cp /model/cached_train_roberta-base_384 ./ && python run_squad.py --model_type roberta --model_name_or_path /model/roberta-base --do_train --do_lower_case --train_file /data/SQuAD/train-v1.1.json --predict_file /data/SQuAD/dev-v1.1.json --per_gpu_train_batch_size ${batch_size} --per_gpu_eval_batch_size ${batch_size} --learning_rate 3e-5 --num_train_epochs 1.0 --iters 30  --max_seq_length 384 --doc_stride 128 --output_dir ./outputs_bert --overwrite_output_dir --gradient_accumulation_steps 1 --device gpu" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
  elif [[ ${dev_type} == "mlu" ]]; then
    docker exec -it "${CONT_NAME}" bash -c "export MLU_VISIBLE_DEVICES=0 && cp /model/cached_train_roberta-base_384 ./ && source /torch/venv3/pytorch/bin/activate && python run_squad.py --model_type roberta --model_name_or_path /model/roberta-base --do_train --do_lower_case --train_file /data/squad/v1.1/train-v1.1.json --predict_file /data/squad/v1.1/dev-v1.1.json --per_gpu_train_batch_size ${batch_size} --per_gpu_eval_batch_size ${batch_size} --learning_rate 3e-5 --num_train_epochs 1.0 --iters 30  --max_seq_length 384 --doc_stride 128 --output_dir ./outputs_bert --overwrite_output_dir --gradient_accumulation_steps 1 --device mlu" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
  fi
else
  if [[ ${dev_type} == "gpu" ]]; then
    docker exec -it "${CONT_NAME}" bash -c "cp /model/cached_train_roberta-base_384 ./ && python -m torch.distributed.launch --nproc_per_node=${device_count} run_squad.py --model_type roberta --model_name_or_path /model/roberta-base --do_train --do_lower_case --train_file /data/SQuAD/train-v1.1.json --predict_file /data/SQuAD/dev-v1.1.json --per_gpu_train_batch_size ${batch_size} --per_gpu_eval_batch_size ${batch_size} --learning_rate 3e-5 --num_train_epochs 1.0 --iters 30  --max_seq_length 384 --doc_stride 128 --output_dir ./outputs_bert --overwrite_output_dir --gradient_accumulation_steps 1 --device gpu" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log    
  elif [[ ${dev_type} == "mlu" ]]; then
    docker exec -it "${CONT_NAME}" bash -c "cp /model/cached_train_roberta-base_384 ./ && source /torch/venv3/pytorch/bin/activate &&  python -m torch.distributed.launch --nproc_per_node=${device_count} run_squad.py --model_type roberta --model_name_or_path /model/roberta-base --do_train --do_lower_case --train_file /data/squad/v1.1/train-v1.1.json --predict_file /data/squad/v1.1/dev-v1.1.json --per_gpu_train_batch_size ${batch_size} --per_gpu_eval_batch_size ${batch_size} --learning_rate 3e-5 --num_train_epochs 1.0 --iters 30  --max_seq_length 384 --doc_stride 128 --output_dir ./outputs_bert --overwrite_output_dir --gradient_accumulation_steps 1 --device mlu" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log    
  fi
fi
