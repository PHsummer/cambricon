...
then
  if [[ ${dev_type} == "gpu" ]]; then
    docker exec -it "${CONT_NAME}" bash -c "python ShuffleNetV1/train.py --model-size 1.0x --train-dir /data/imagenet_training --val-dir /data/imagenet_training --batch-size ${batch_size} --total-iters 80" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
  elif [[ ${dev_type} == "mlu"  ]]; then
    docker exec -it "${CONT_NAME}" bash -c "source /torch/venv3/pytorch/bin/activate && python ShuffleNetV1/train.py --model-size 1.0x --train-dir /data/imagenet_training --val-dir /data/imagenet_training --batch-size ${batch_size} --total-iters 80" 2>&1 | tee ${LOG_DIR}/${model_name}_${DATESTAMP}.log
  fi
else
...
cost=$(grep "TRAIN Iter 40" ${LOG_DIR}/${model_name}_${DATESTAMP}.log | cut -d = -f 7 |cut -d " " -f 2 |head -c 6)
