...
def _get_cpu_info():
    ...
    for line in all_info.split("\n"):
        if "Model name" in line:
            ...
            cpu_name = line.rpartition('：       ')[-1]
        elif line.startswith("座"):
            cpu_phy_count = line.rpartition('：             ')[-1]
        elif line.startswith("每个座的核数"):
            cpu_logic_count = line.rpartition('：   ')[-1]
...
def _get_gpu_info():
    all_info = _get_cmd_info("nvidia-smi -L")
    dev_info = {}
    ...
    try:
        for line in all_info.split("\n"):
            if "GPU" in line:
                dev_name_tmp = re.sub("GPU .:", "", line, 1).strip()
                dev_name = re.sub(r'\(.*\)', "", dev_name_tmp, 1).strip()
                obj_add(dev_name, dev_info)
            elif "LnkCap" in line:
                lnkcap = re.sub(".*LnkCap.*:", "", line, 1).strip()
                obj_add(lnkcap, dev_info)
...
def get_gpu_name():
    ...
    Get device name from 'nvidia-smi -L'
    ...
    for line in all_info.split("\n"):
        if "GPU" in line:
            temp = re.sub("GPU .:", "", line, 1).strip()
            return re.sub(r'\(.*\)', "", temp, 1).strip()
    return "unknown"
...
def dump_gpu_machine_info(name=None, file=None):
    """Dump GPU Machine information to STDOUT or file in YAML format.
    ...
    - cpu: cpu information, include socket number, logical core number
    - mem: total memory
    - dev: gpu information, include mlu name, number, pcie capability
    ...
    try:
        ...
        for line in tf_info.split("\n"):
            if "Current library versions" in line:
                ...
                cnml = re.search(r"CNML: ([\d\.]*)", line).group(1)
                # cncl = re.search(r"CNCL: ([\d\.]*)", line).group(1)
