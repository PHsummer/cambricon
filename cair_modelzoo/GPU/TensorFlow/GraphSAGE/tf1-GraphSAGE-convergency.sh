...
# Run experiments
docker exec -it "${CONT_NAME}" bash -c "rm -rf ./data && ln -s /data/tensorflow/ppi && CUDA_VISIBLE_DEVICES=0 python -m graphsage.unsupervised_train --train_prefix /data/ppi/ppi --model graphsage_mean --epochs 10 && python eval_scripts/ppi_eval.py /data/ppi unsup-ppi/graphsage_mean_small_0.000010 test" 2>&1 | tee ${LOG_DIR}/GraphSAGE-convergency_${DATESTAMP}.log 
