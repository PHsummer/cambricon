...
from utils.preprocess import color_shift
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        # print([x.split()[:-1] for x in f.read().strip().splitlines()])
                        l = np.array([x.split()[:13] for x in f.read().strip().splitlines()], dtype=np.float32)  # labels
                    #print(l.shape[1])
                    if len(l):
