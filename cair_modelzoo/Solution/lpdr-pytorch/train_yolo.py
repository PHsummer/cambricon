...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    opt.train_path = opt.data_path+"/train"
    opt.test_path = opt.data_path+"/test"
    nc = opt.class_nums   # number of classes
    names = opt.class_names
    ...
    dataloader, dataset = create_dataloader(opt.train_path, imgsz, batch_size, gs, opt,
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(opt.test_path, imgsz_test, total_batch_size, gs, opt,  # testloader
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]: # and epoch > 1:
            ...
            if save:
                ...
                print("Last Model save:", last)
                if best_fitness == fi:
                    torch.save(ckpt, best)
                    print("Best Model save:", best)
                del ckpt
        # end epoch ----------------------------------------------------------------------------------------------------
    # end training
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/workspace/LPDR/Database/DB_Detection', help='dataset path')
    parser.add_argument('--class_nums', type=int, default=1)
    parser.add_argument('--class_names', type=list, default=['plate'])
    ...
    parser.add_argument('--project', default='./checkpoints/yolov5', help='save to project/name')
