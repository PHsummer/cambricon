...
import argparse
import onnxruntime
from utils.tensorrt_infer import *
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            if args.mode=="pth":
                ...
                t1 = time.time()
                output = model.run([], {'input': images_input})
                t2 = time.time()
                output = torch.from_numpy(np.array(output[0])).cuda()
                infer_time = t2-t1

            elif args.mode=="trt":
                images_input = images.cpu().numpy()
                output, infer_time = infer(model, images_input)
                output = torch.from_numpy(np.array(output)).cuda()

            infertime.append(infer_time/args.batch_size)
...
def load_engine(filename: str):
    ...
    print("Loaded engine: {}".format(filename))
...
if __name__ == '__main__':
    ...
    if args.mode=="pth":
        ...
        model = onnxruntime.InferenceSession(args.resume, None)
    elif args.mode=="trt":
        model = load_engine(args.resume)
