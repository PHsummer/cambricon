...
def main():
    ...
    parser.add_argument("--max-calibration-size", help="(INT8 ONLY) The max number of data to calibrate on from --calibration-data.", type=int, default=0)
    ...
    with trt.Builder(TRT_LOGGER) as builder, \
        ...
        if args.explicit_batch:
            ...
            profile = builder.create_optimization_profile()
            profile.set_shape("input", min=(args.max_batch_size,3,224,224), opt=(args.max_batch_size,3,224,224), max=(args.max_batch_size,3,224,224))
            config.add_optimization_profile(profile)
